...
│  - Periodic meta-introspection & weight remapping       │
...
Periodic **meta-introspection** (every N generations) examines slow-timescale signals — plateau duration, novelty, diversity, motif entropy, and chaos rhythm — to reshape motive weights at a higher level of abstraction. The agent distinguishes between being stuck-and-redundant (→ explore more) versus stuck-but-already-wandering (→ exploit more).
...
A salience memory tracks tripeptide motifs weighted by their association with high-fitness outcomes, with exponential decay. This creates an attention-like mechanism: motifs that historically produce good peptides receive preferential treatment during mutation, while the system remains capable of exploring new motifs when those become stagnant.
...
To run the standard GA baseline for comparison:

```bash
# Set USE_AGENT = False in evolve.py, then:
python scripts/evolve.py
```

**Graceful early termination** — to stop a long run without losing progress:
...
```bash
python scripts/analyze_agent.py   # Agent behavior diagnostics
python scripts/analyze.py         # General evolution diagnostics
python scripts/extract.py         # Extract top candidates
```
...
peptide-evolution/
├── scripts/
│   ├── evolve.py                 # Main evolution loop (agent ON/OFF)
│   ├── evolveAGENT.py            # Agent-focused variant
│   ├── train_amp_model_v2.py     # AMP CNN training
│   ├── train_toxicity_model_v2.py
│   ├── train_stability_model_v2.py
│   ├── train_naturalness_model.py
│   ├── train_prophet.py          # Prophet mutation model training
│   ├── analyze_agent.py
│   ├── analyze.py
│   └── extract.py
├── models/                       # Trained .keras model files (not included)
├── external/
│   ├── PyAMPA/                   # External AMP/toxicity/hemolysis models
│   └── pyampa_integration.py
├── data/
│   ├── model_trainers/           # Training datasets
...
Questions, collaborations, or feedback welcome.  
